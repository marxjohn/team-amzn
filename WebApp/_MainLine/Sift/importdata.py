...
def commitPost(post, conn):
        ...
        cur.execute(query, (post[0], post[1], post[2], post[3], post[4], post[5], post[6], post[7], post[8], post[9], post[10], post[11], post[12], post[13], post[14], post[15]))
        conn.commit()
    ...
    for f in DATA_FILES:
        ...
        with open(f, encoding="utf-8", errors="surrogateescape") as data_file:
            while True:
                ...
                data = [parse(data_file.readline()).split('=', 2)[1].rstrip().encode('ascii', 'replace')
                        for i in range(17)][:-1]
                data[13] = convertTime(data[13].decode('utf-8'))
                data[14] = convertTime(data[14].decode('utf-8'))
                if data[0]:
                    i += 1
                ...
                if (i-j == 0):
                    p += 5
                    j += 300
                    print("% of file: " + str(p) + " post: " + str(i) + " time: " + str(round(time()-t0, 2)))
